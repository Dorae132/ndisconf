...
import com.nsb.ndisconf.server.disconf.web.config.ApplicationPropertyConfig;
import com.nsb.ndisconf.server.disconf.web.innerapi.zookeeper.ZooKeeperDriver;
...
import com.nsb.ndisconf.server.disconf.web.service.config.form.ConfListForm;
...
import com.nsb.ndisconf.server.disconf.web.service.config.vo.ConfListVo;
import com.nsb.ndisconf.server.disconf.web.service.config.vo.MachineListVo;
...
import com.nsb.ndisconf.server.disconf.web.service.zookeeper.dto.ZkDisconfData;
import com.nsb.ndisconf.server.disconf.web.service.zookeeper.dto.ZkDisconfData.ZkDisconfDataItem;
import com.nsb.ndisconf.server.disconf.web.service.zookeeper.service.ZkDeployMgr;
import com.nsb.ndisconf.server.disconf.web.utils.DiffUtils;
import com.nsb.ndisconf.server.disconf.web.utils.MyStringUtils;
import com.nsb.ndisconf.server.dsp.common.constant.DataFormatConstants;
import com.nsb.ndisconf.server.dsp.common.utils.DataTransfer;
import com.nsb.ndisconf.server.dsp.common.utils.ServiceUtil;
import com.nsb.ndisconf.server.dsp.common.utils.email.LogMailBean;
...
public class ConfigMgrImpl implements ConfigMgr {
	...
	public String updateItemValue(Long configId, String value) {
		...
		if (applicationPropertyConfig.isEmailMonitorOn()) {
			boolean isSendSuccess = logMailBean.sendHtmlEmail(toEmails, " config update",
					...
					//DiffUtils.getDiff(CodeUtils.unicodeToUtf8(oldValue), value, config.toString(), getConfigUrlHtml(config)));
			if (isSendSuccess) {
				return "修改成功，邮件通知成功";
			} else {
	...
	public void deleteAppConfig(Long appid) {
		...
		if (envs != null && envs.size() > 0) {
			for (Env env : envs) {
				List<Config> cfgs = configDao.getConfByAppEnv(appid, env.getId());
